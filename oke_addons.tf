resource "oci_containerengine_addon" "myapp_oke_addon" {
    for_each = local.myapp_oke_addon_map  
    addon_name = each.key
...
resource "oci_containerengine_addon" "myapp_oke_autoscaler_addon" {
    ...
    configurations {
      key = "nodes"
      value = join("", [var.autoscaler_min_number_of_nodes,":",var.autoscaler_max_number_of_nodes,":",oci_containerengine_node_pool.myapp_oke_autoscaler_node_pool[0].id])
    }
