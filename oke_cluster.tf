...
resource "oci_containerengine_cluster" "myapp_oke_cluster" {
  compartment_id     = var.compartment_ocid
  kubernetes_version = var.k8s_version
  name               = var.oke_cluster_name
  vcn_id             = var.use_existing_vcn ? var.vcn_id : oci_core_vcn.myapp_oke_vcn[0].id
  ...
  dynamic "endpoint_config" {
    ...
    content {
      ...
      subnet_id            = var.use_existing_vcn ? var.api_endpoint_subnet_id : oci_core_subnet.myapp_oke_api_endpoint_subnet[0].id
  ...
  options {
    service_lb_subnet_ids = [var.use_existing_vcn ? var.lb_subnet_id : oci_core_subnet.myapp_oke_lb_subnet[0].id]
