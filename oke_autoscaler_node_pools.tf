resource "oci_containerengine_node_pool" "myapp_oke_autoscaler_node_pool" {
  depends_on         = [oci_containerengine_node_pool.myapp_oke_node_pool]
  count              = var.virtual_node_pool ? 0 : var.autoscaler_enabled ? var.autoscaler_node_pool_count : 0
  cluster_id         = oci_containerengine_cluster.myapp_oke_cluster.id
  ...
  dynamic "node_source_details" {
    ...
    content {
      ...
      source_type = data.oci_containerengine_node_pool_option.myapp_oke_node_pool_option.sources[0].source_type
    }
  }
  ...
  node_config_details {
    dynamic "placement_configs" {
      ...
      content {
        ...
        subnet_id           = var.use_existing_vcn ? var.nodepool_subnet_id : oci_core_subnet.myapp_oke_nodepool_subnet[0].id
      }
    }
    size = var.node_count
   # defined_tags  = { "oke.pool" = "autoscaler" }
    ...
    dynamic "node_pool_pod_network_option_details" {
        ...
        content {
          ...
          pod_subnet_ids    = var.use_existing_vcn ? [var.pods_subnet_id] :  [oci_core_subnet.myapp_oke_nodepool_subnet[0].id]
        }
    }
  }
